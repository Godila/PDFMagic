...
2. ТОЧНОСТЬ ЗНАЧЕНИЙ
   ...
   - НЕ суммируй значения по разным зданиям/очередям — сохраняй каждое отдельно через " / "
...
3. НЕСКОЛЬКО ЗНАЧЕНИЙ (здания, очереди, пусковые комплексы)
   ...
   - Пример для "Площадь здания": "6918 кв.м\n2292 кв.м\n9210 кв.м" (не 18420 кв.м!)
   - Пример для "Паркинг": "960 м/м\n235 м/м"
...
6. УЧРЕЖДЕНИЯ И НОРМАТИВЫ
   - Для ДОО, СОШ, медицины и т.д. — указывай только основное значение (количество мест/коек)
   - Площадь участка под объект (~X га) указывай отдельно если есть
